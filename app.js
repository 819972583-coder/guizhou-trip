...
function navigationDestinations(item) {
  const rawPolicy = state.data.mapLinks?.navigationPolicy;
  const policy = (rawPolicy && typeof rawPolicy === "object")
    ? { noNavigationTypes: rawPolicy.noNavigationTypes || [], selfNavigationTypes: rawPolicy.selfNavigationTypes || [] }
    : { noNavigationTypes: [], selfNavigationTypes: [] };
...
function rentalStatus(rental) {
  ...
  const now = new Date();
  if (now < pickup) return { label: "距用车开始", target: pickup, complete: false };
  if (now < dropoff) return { label: "距用车结束", target: dropoff, complete: false };
  return { label: "已超过预约用车结束时间", target: dropoff, complete: true };
}
...
function renderRental() {
  ...
  const priceProvided = price.payAtCounter !== null && price.payAtCounter !== undefined && Number.isFinite(Number(price.payAtCounter));
  $("#rental-card").innerHTML = `
    <article class="rental-panel">
      <div class="return-deadline">
        <span class="return-deadline__label">重要 · 用车截止时间</span>
        ...
        <small>建议 ${escapeHtml(rental.dropoff.recommendedArrivalTime)} 抵达终点，预留交接时间。</small>
      </div>
      ...
      <div class="rental-details">
        ...
        <div class="rental-stops">
          ...
          <div class="rental-stop">
            ...
            <div><b>${formatCompactDate(rental.dropoff.date)} ${escapeHtml(rental.dropoff.time)}</b><span>${escapeHtml(rental.dropoff.vehicleReturnPoint)}<br>建议 ${escapeHtml(rental.dropoff.recommendedArrivalTime)} 抵达终点</span></div>
          </div>
        </div>
        <div class="rental-price"><span>${priceProvided ? `柜台支付 · ${rental.rentalPeriodDays} 天` : "费用"}</span><strong>${priceProvided ? `${escapeHtml(price.currency)} ${Number(price.payAtCounter).toFixed(2)}` : "未提供"}</strong></div>
      </div>
    </article>
  `;
  ...
  notes.innerHTML = `
    <div class="drive-note-tabs" role="group" aria-label="租车注意事项">
      <button type="button" aria-expanded="true" aria-controls="drive-note-content" data-drive-note="checklist">用车检查</button>
...
async function init() {
  try {
    if (window.__TRAVEL_DATA__) {
      state.data = window.__TRAVEL_DATA__;
    } else {
    ...
    applyModuleConfig();
    const safe = (label, fn) => {
      try { fn(); } catch (error) { console.error(`模块渲染失败：${label}`, error); }
    };
    if (moduleEnabled("overview")) safe("地图底图预载", preloadDefaultRouteMap);
    safe("Hero", renderHero);
    if (moduleEnabled("flights")) safe("航班", renderFlights);
    if (moduleEnabled("overview")) safe("路线探索", setupRouteExplorer);
    if (moduleEnabled("itinerary")) {
      safe("地图弹层", setupPlaceMap);
      safe("门票弹层", setupTicketDialog);
    }
    ...
    if (moduleEnabled("itinerary")) safe("每日行程", renderTimeline);
    if (moduleEnabled("driving")) safe("租车", renderRental);
    if (moduleEnabled("todo")) safe("旅行前准备", renderTravelPrep);
    if (moduleEnabled("ledger")) {
      try { await window.TravelLedger?.init?.({ tripId: state.data.metadata.tripId, config: state.config }); }
      catch (error) { console.error("记账模块初始化失败", error); }
    }
    safe("倒计时", startCountdowns);
  } catch (error) {
